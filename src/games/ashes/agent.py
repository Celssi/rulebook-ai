"""Ashes agent graph nodes."""

from __future__ import annotations

from src.games.ashes.actions import SHORTCUT_IDS
...
def ashes_multi_node(state: dict) -> dict:
    game_id = state.get("game_id", GAME_ASHES)
    shortcut_id = state.get("shortcut_id") or "draw_room"
    ...
    from src.games.ashes.play_handlers import run_scion_shortcut

    ctx = load_play_context_from_disk(store, char_id)
    user_message, answer, sources, _route = run_scion_shortcut(
        ctx,
        shortcut_id,
