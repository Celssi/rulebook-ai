"""Lighthouse agent graph node."""

from __future__ import annotations

from src.games.lighthouse.actions import SHORTCUT_IDS
...
def lighthouse_multi_node(state: dict) -> dict:
    game_id = state.get("game_id", GAME_LIGHTHOUSE)
    shortcut_id = state.get("shortcut_id") or "order_of_play"
    ...
    user_message, answer, sources, _route = run_watch_shortcut(
        ctx,
        shortcut_id,
