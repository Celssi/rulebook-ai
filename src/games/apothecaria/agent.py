"""Apothecaria agent graph nodes."""

from __future__ import annotations

from src.games.apothecaria.actions import SHORTCUT_IDS
...
def apothecaria_multi_node(state: dict) -> dict:
    game_id = state.get("game_id", GAME_APOTHECARIA)
    shortcut_id = state.get("shortcut_id") or "draw_ailment"
    ...
    from src.games.apothecaria.play_handlers import run_cottage_shortcut

    ctx = load_play_context_from_disk(store, char_id)
    user_message, answer, sources, _route = run_cottage_shortcut(
        ctx,
        shortcut_id,
