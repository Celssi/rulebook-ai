...
from api.utils import recent_chat_history, to_langchain_history
...
def _store():
    return get_play_store(GAME_BRAMBLETREK)
...
def answer_table_lookup_prompt(
    ...
    *,
    ...
    result = rag_query(
        ...
        brambletrek_character=get_character(ctx),
...
def execute_shortcut(
    ...
    *,
    ...
    run = run_shortcut(shortcut_id, game_id=GAME_BRAMBLETREK, **shortcut_kwargs(ctx))
    kind = run["kind"]
    ...
    out = run_agent(
        ...
        brambletrek_character=ctx.entity,
        chat_provider=chat_provider,
        char_id=ctx.slot_id or None,
...
def switch_gnawborn(app: AppSession, char_id: str) -> PlayContext:
    ...
    ctx = app.play_context(GAME_BRAMBLETREK)
    new_ctx = store.switch_slot_ctx(ctx, char_id)
    app.play[GAME_BRAMBLETREK] = new_ctx
    return new_ctx
